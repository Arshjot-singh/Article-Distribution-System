...
def parse_godown_stock(file):
    ...
    if file.name.endswith('.pdf'):
        ...
        if 'article' in df.columns and 'quantity' in df.columns:
            ...
            st.error("CSV should have 'article' and 'quantity' columns for godown stock data.")
            return {}
    else:
...
def parse_articles_sent(file):
    ...
    if file.name.endswith('.pdf'):
        ...
        if 'store' in df.columns and 'article' in df.columns:
            ...
            st.error("CSV should have 'store' and 'article' columns for articles sent data.")
...
def parse_store_capacities(file):
    ...
    if file.name.endswith('.pdf'):
        ...
        if 'store' in df.columns and 'capacity' in df.columns:
            ...
            st.error("CSV should have 'store' and 'capacity' columns for store capacities.")
            capacities = {}
